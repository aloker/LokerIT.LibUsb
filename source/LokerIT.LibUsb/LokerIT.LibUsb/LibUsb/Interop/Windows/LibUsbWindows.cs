...
namespace LokerIT.LibUsb.Interop.Windows
{
    public class LibUsbWindows : ILibUsb, IDisposable
    {
        private readonly ILogger<LibUsbWindows> _logger;
        private IntPtr _addedDllDirectory;

        public LibUsbWindows(ILogger<LibUsbWindows> logger)
        {
            _logger = logger;
            ...
            var baseDir = Path.GetDirectoryName(new Uri(Assembly.GetExecutingAssembly().CodeBase).AbsolutePath);

            switch (arc)
            {
                case Architecture.X64:
                    _addedDllDirectory =
                        Native.AddDllDirectory(Path.Combine(baseDir, "x64"));
                    break;
                case Architecture.X86:
                    _addedDllDirectory =
                        Native.AddDllDirectory(Path.Combine(baseDir, "x86"));
                    break;
                default:
                    _logger.LogCritical("Platform {OsPlatform} {ProcessArchitecture} is not supported",
                        OSPlatform.Windows, arc);
                    throw new NotSupportedException(
                        $"{OSPlatform.Windows} {arc} is not supported");
            }

            Native.SetDefaultDllDirectories(Native.DirectoryFlags.DefaultDirs);
            ...
            _logger.LogInformation("libusb {Version} loaded", version);
        }
        ...
        {
            if (_addedDllDirectory == IntPtr.Zero) return;
            _logger.LogDebug("Releasing DLL directory");
            Native.RemoveDllDirectory(_addedDllDirectory);
            _addedDllDirectory = IntPtr.Zero;
        }
